...
async function fetchMovieDetails(movieId) {
  try {
    const response = await fetch(
      `https://learn-with-guvi-book-my-show-backend-06ol.onrender.com/movie/${movieId}`
    );
...
document.getElementById("submitBooking").addEventListener("click", async () => {
  ...
  try {
    const response = await fetch(
      "https://learn-with-guvi-book-my-show-backend-06ol.onrender.com/movie/book-movie",
      {
        method: "POST",
        headers: {
